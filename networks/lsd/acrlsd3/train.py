...
lsd_checkpoint = '../lsd/model_checkpoint_300000'
...
def mknet():
    ...
    lsd_in_channels = 1 # raw
    acrlsd_in_channels = 11 # raw + lsds
    num_fmaps = 12
    fmap_inc_factor = 5
...
def train_until(max_iteration):
    ...
    cropped_raw = ArrayKey('CROPPED_RAW')
    ...
    input_shape = Coordinate((72,300,300))
    intermediate_shape = Coordinate((52,208,208))
    ...
    request.add(cropped_raw, intermediate_size)
    ...
    train_pipeline += CropRaw(raw, cropped_raw)
    ...
    train_pipeline += Unsqueeze([raw, cropped_raw])
    ...
    train_pipeline += Squeeze([raw, cropped_raw])
    train_pipeline += Squeeze([raw, cropped_raw, pred_lsds, gt_affs, pred_affs])

...
if __name__ == "__main__":
    ...
    tree_json = json.load(open(folder + "/annotations/cleft2pre/tree_geometry.json", 'r'))
    ...
    for skeleton in tree_json['skeletons']:
        ...
        for synapse_id in tree_json['skeletons'][skeleton]["connectors"]:
            ...
            synapse_db[synapse_id]['location'] = (
                synapse['location'][2],
                synapse['location'][1],
                synapse['location'][0],
                )
    ...
    # print(synapse_db)
    out = folder + "/annotations/cleft2pre/ground_truth.csv"
    n = 0
    with open(out, 'w') as file:
        ...
        for synapse in synapse_db:
            ...
            if 'postsynaptic_to' not in synapse:
                continue
            n += 1
            pre_zyx = synapse['location']
            post_zyx = pos_db[synapse['presynaptic_to']]
            # pre_zyx = pos_db[synapse['presynaptic_to']]
            # post_zyx = pos_db[synapse['postsynaptic_to']]
            # print("synapse:", synapse)
            # print("pre_zyx:", pre_zyx)
            # print("post_zyx:", post_zyx)
            file.write(
                str(pre_zyx[0]) + ',' +
                str(pre_zyx[1]) + ',' +
                str(pre_zyx[2]) + ',' +

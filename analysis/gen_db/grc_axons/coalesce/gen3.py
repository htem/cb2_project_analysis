...
z_length_errata = False
z_length_correction = 40
...
def average_syns(syn_list):
    ...
    if 'major_axis_length' in new_syn:
        new_syn['major_axis_length'] = max([s['major_axis_length'] for s in syn_list])

    new_syn['props']['z_length'] = sum([s['props']['z_length'] for s in syn_list])

    if 'props' in syn_list[0]:
        new_props = {}
        keys = syn_list[0]['props'].keys()
        for k in keys:
            if isinstance(syn_list[0]['props'][k], dict):
                new_props[k] = {}
                for kk in syn_list[0]['props'][k].keys():
                    if kk == 'skeleton_longest_diameter':
                        psum = 0
                        for i in range(len(syn_list)):
                            if kk in syn_list[i]['props'][k]:
                                psum = max(psum, syn_list[i]['props'][k][kk])
                        new_props[k][kk] = psum
                        ...
                        for i in range(len(syn_list)):
                            if kk in syn_list[i]['props'][k]:
                                psum += syn_list[i]['props'][k][kk]
                        new_props[k][kk] = psum
            else:
                psum = 0
                for i in range(len(syn_list)):
                    psum += syn_list[i]['props'][k]
                new_props[k] = psum
        new_syn['props'] = new_props
    return new_syn
...
if '210429' in in_fname:
    z_length_errata = True
else:
...
for grc, grc_syns in syndb.items():
    ...
    for pc, syns in grc_syns.items():
        for s in syns:
            # print(s)
            s['sf_pre'] = int(s['sf_pre'])
            s['sf_post'] = int(s['sf_post'])
            # print(s)
            if z_length_errata:
                s['props']['z_length'] -= z_length_correction 
            # for k in s['props']:
            #     if s['props'][k]
        post_locs = [s['post_loc'] for s in syns]
        post_to_syn = dict()
        for s in syns:

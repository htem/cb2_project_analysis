...
with open('grc_with_axons_orphan_segments', 'w') as fout:
...
grc_box_segs = defaultdict(lambda: defaultdict(list))
for grc_id in grcs:
    ...
    for sid in grc.segments:
        if sid == 0:
            continue
        grc_box_segs[grc_id][getBoxId(sid)].append(sid)
...
for pc_id in pc_list:
    ...
    for boxid in local_pc_vert_by_box:
        ...
        for grc_id in grc_box_segs:
            segs = grc_box_segs[grc_id][boxid]
...
with open('grc_pc_touches_200923', 'w') as fout:
...
'''pickle'''
fname = 'grc_pc_touches_200923.gz'

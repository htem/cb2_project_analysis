...
def test_snr_across_noise(
    ...
    for input_noise_pct in loop_vals:
        ...
        for synapse_pct in synapse_pcts:
            synapse_weights = get_optimal_weights3(grc_scores, synapse_pct)

            # calculate signals for the test patterns
            signals_ = []
            for test_pattern in grc_acts_test:
                signals_.append(calc_signal(test_pattern, synapse_weights))
            ...
            signals = [s-bg_mean for s in signals_]
...
def get_optimal_weights3(scores, synapse_pct):
    '''Make synapse where scores are highest'''
    num_synapse = int(synapse_pct/100*len(scores))
    assert num_synapse > 0
    weights = [0]*len(scores)
    # num_synapse = 10

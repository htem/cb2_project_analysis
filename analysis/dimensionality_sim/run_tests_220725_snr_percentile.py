...
def test_snr_across_noise(
        ...
        synapse_pct_delta=0.5,
        ):

    assert pattern_size_pct + unrelated_mfs_pct <= 100.0

    # if synapse_pcts is None:
    #     synapse_pcts = [synapse_pct]
    ...
    for input_noise_pct in loop_vals:
        ...
        for grc_act in grc_acts_train:
            for i, b in enumerate(grc_act):
                if b:
                    grc_scores[i] += 1
        grc_scores = list(enumerate(grc_scores))
        random.shuffle(grc_scores)  # to randomly break ties
        grc_scores = sorted(grc_scores, key=lambda x: x[1], reverse=True)

        # print(grc_scores); exit()
        # print([k[1] for k in grc_scores]); exit()
        ...
        percentiles = make_pct_seq(0.0, 0.6, synapse_pct_delta/100)
        # print(percentiles); exit()
        # percentiles = range(0, 60, .5)

        rets_syn = {}

        # for synapse_pct in synapse_pcts:
        for synapse_pct in percentiles:
            print(synapse_pct)
            pct1 = synapse_pct + synapse_pct_delta
            # synapse_weights = get_optimal_weights2(grc_scores, synapse_pct)
            synapse_weights = get_optimal_weights2_percentile(
                                grc_scores, synapse_pct, pct1)
            ...
            signals = [s-bg_mean for s in signals]
            signal_size = abs(sum(signals)/len(signals))
            signal_std = statistics.stdev(signals)

            snr = signal_size / bg_std
        ...
        rets_noise[input_noise_pct] = rets_syn
        # if len(synapse_pcts) == 1:
        #     rets_noise[input_noise_pct] = rets_syn[synapse_pct]
...
def get_optimal_weights2(grc_scores, synapse_pct):
    ...
    # print(grc_scores)
    num_synapse = int(synapse_pct/100*len(grc_scores))
    # print(num_synapse); asdf
    assert num_synapse > 0
    weights = [0]*len(grc_scores)
    # num_synapse = 10
    for i, s in grc_scores:
        weights[i] = 1
        num_synapse -= 1
        if num_synapse == 0:
            break
...
def get_optimal_weights2_percentile(grc_scores, pct0, pct1):
    '''Make synapse where grc_scores are highest'''
    assert pct1 >= pct0
    index = int((pct0)/100*len(grc_scores))
    num_synapse = int((pct1-pct0)/100*len(grc_scores))
    assert num_synapse > 0
    weights = [0]*len(grc_scores)
    for i, s in grc_scores[index:]:
        weights[i] = 1
        num_synapse -= 1
        if num_synapse == 0:
            break

...
def make_graph(model, n_grcs, n_mfs,
    ...
    #     model_desc = f'{model}'
    ...
    elif model == 'local_random_expanded2':

        overgrow_factor = config_.get('overgrow_factor', 0.0)
        assert(overgrow_factor > 0.0)
        clumpy_mfs = config_.get('clumpy_mfs', 0)
        increase_sharing = config_.get('increase_sharing', 0)
        ...
        g.randomize_graph_by_grc2(
            mf_dist_margin=5000,
            single_connection_per_pair=1,
            overgrow=overgrow_factor,
            preserve_mf_degree=clumpy_mfs,
            seed=seed,
            )

        tools_mf_graph.remove_overgrowth(g, overgrowth=overgrow_factor, no_scoring=(not increase_sharing))
        model_desc = f'{model}_{overgrow_factor}_{increase_sharing}_{clumpy_mfs}'
        ...
        tools_mf_graph.remove_overgrowth(g, overgrowth=overgrow_factor)
        model_desc = f'{model}'

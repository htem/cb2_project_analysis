...
def make_random_mask(length, f, additional_mask=None):
    if additional_mask is not None:
        # need to adjust f so that once merged the effective `f` stays the same
        additional_mask_f = sum(additional_mask)/len(additional_mask)
        keep_f = 1-f
        keep_f = keep_f / (1 - additional_mask_f)
        f = 1 - keep_f
    mask = np.zeros(length, dtype=np.uint8)
    mask[0:int(f*length)] = 1
...
def add_noise_patterns_using_masks(
        patterns,
        pattern_masks,
    ...
    if pattern_type == "binary":
        return add_noise_patterns_using_masks_binary(patterns, pattern_masks, **kwargs)
    elif pattern_type == "uniform":
        return add_noise_patterns_using_masks_uniform(patterns, pattern_masks, **kwargs)
    else:
        raise RuntimeError("Unimplemented pattern_type `{pattern_type}`")


def add_noise_patterns_using_masks_binary(
        patterns,
        pattern_masks,
        f=None,
        input_noise_pct=0,
    ...
    # pattern_masks is 1 where there should be changes
    input_noise_prob = input_noise_pct/100
    assert input_noise_prob <= 1.0
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    pattern_len = len(patterns[0][0])
    if f is None:
        n = min(4, len(patterns))
        f = 0.0
        for i in range(n):
            f += sum(patterns[i][0]) / pattern_len
        f /= n
    assert f <= 1.0

    random_pattern = np.zeros(pattern_len, dtype=np.uint8)
    random_pattern[0:int(f*pattern_len)] = 1

    noise_mask = np.zeros(pattern_len, dtype=np.uint8)
    noise_mask[0:int(input_noise_prob*pattern_len)] = 1
    noise_pattern = np.zeros(pattern_len, dtype=np.uint8)
    noise_pattern[0:int(f*pattern_len)] = 1

    ret = []
    for pattern, mask in zip(patterns, pattern_masks):
        pattern, output = pattern
        for i in range(test_mult):
            np.random.shuffle(random_pattern)
            new_pattern = np.where(mask, random_pattern, pattern)
            if input_noise_pct:
                np.random.shuffle(noise_mask)
                np.random.shuffle(noise_pattern)
                new_pattern = np.where(noise_mask, noise_pattern, new_pattern)
            ret.append((new_pattern, output))
    return ret

def add_noise_patterns_using_masks_uniform(
        patterns,
        pattern_masks,
        f=None,
        input_noise_pct=0,
    ...
    random_pattern = np.random.random(size=pattern_len)
    noise_pattern = np.random.random(size=pattern_len)
    ...
    for pattern, mask in zip(patterns, pattern_masks):
        ...
        for i in range(test_mult):
            ...
            if input_noise_pct:
                np.random.shuffle(noise_pattern)
                new_pattern = new_pattern*(1-input_noise_prob) + noise_pattern*input_noise_prob
            ret.append((new_pattern, output))
    return ret

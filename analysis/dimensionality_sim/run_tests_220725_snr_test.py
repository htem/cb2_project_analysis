...
import functools
import random_patterns2
import time
import numpy as np
import os
import statistics
from random_patterns2 import generate_patterns, make_random_mask
...
default_pattern_generator = random_patterns2.generate_patterns
...
def test_snr_across_noise(
    ...
    # make random synapse mask now for deterministic addition across iterations
    # random_synapse_mask0 = make_random_mask(sim.num_grcs, 0.5)
    # synapse_addition_order = np.where(random_synapse_mask0 == 1)[0]
    synapse_addition_order = list(range(sim.num_grcs))
    random.shuffle(synapse_addition_order)
    ...
    for input_noise_pct in loop_vals:
        ...
        for synapse_pct in synapse_pcts:
            ...
            # merge with random synapse mask
            synapse_weights = add_to_mask(
                synapse_weights, synapse_addition_order, int(sim.num_grcs*0.5))
            # print(sum(synapse_weights))
...
def add_to_mask(mask, order, count):
    count -= sum(mask)
    assert count >= 0
    i = 0
    while count:

...
def test_snr_across_noise(
    ...
    # bg_patterns = pattern_generator(sim.num_mfs, count=512)
    # test_patterns = [patterns0[0]]
    ...
    # print(f'patterns0: {patterns0}'); 
    # print(f'pattern_masks0: {pattern_masks0}'); asdf

    rets = {}
    for input_noise_pct in loop_vals:
        print(f'\ninput_noise_pct: {input_noise_pct}')

        train_noise = input_noise_pct if noise_during_training else 0.0
        ...
        # print(f'patterns_train: {patterns_train}'); 
        # print(f'patterns_test: {patterns_test}'); asdf
        ...
        # sim.evaluate(bg_patterns, no_random=True)
        # grc_acts_bg = copy.deepcopy(sim.get_grc_activities())
        ...
        for grc_act in grc_acts_train:
            for i, b in enumerate(grc_act):
                if b:
                    grc_scores[i] += 1
        # print(f'grc_scores: {grc_scores}'); exit()

        synapse_weights = get_optimal_weights2(grc_scores, synapse_pct)
        print(f'synapse_weights: {synapse_weights}');
        ...
        print(f'signal_size: {signal_size-bg_mean}')

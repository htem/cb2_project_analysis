...
'''compute shortest dist between boutons of a given mf'''
bucket_size = 5000
distribution = defaultdict(int)
for mf in mfs_bouton_locs:
    if len(mfs_bouton_locs[mf]) < 2:
        continue
    shortest = sys.maxsize
    for i in mfs_bouton_locs[mf]:
        for j in mfs_bouton_locs[mf]:
            if i != j:
                shortest = min(shortest, get_eucledean_dist(i, j))
    shortest = int((shortest+bucket_size/2)/bucket_size)
    shortest = int(shortest*bucket_size/1000)
    distribution[shortest] += 1

# print(distribution)
# for i in sorted(distribution.keys()):
#     print(f'{i},{distribution[i]}')

for i in range(0, max(distribution.keys())+1, 5):
    print(f'{i},{distribution[i]}')

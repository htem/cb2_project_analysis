...
ap.add_argument("--noise_probs", type=float, help='',
                        nargs='+',
                        # default=[.05, .5, .9])
                        # default=[k/10 for k in range(1, 10)])
                        default=[1])
ap.add_argument("--model", type=str, help='', default='scaleup')
...
ap.add_argument("--save_fname", type=str, help='', default=None)
...
if config.test_replicated_input:
    ...
    test_dim_similar_input_and_correlation_by_sharing(
        g,
        # print_output=True,
        # test_len=4096,

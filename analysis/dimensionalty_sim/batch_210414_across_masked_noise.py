...
ap.add_argument("--signal_size", type=float, help='', default=None)
...
# compute signal mask
def get_top_mf_signal_mask(sim, signal_ratio, bottom=False, signal_size=None):
    ...
    count = count[0:mask_len+1]
    # print(count)
    if signal_size is not None:
        signal_size = int(signal_size*len(sim.mfs))
        random.shuffle(count)
        count = count[0:signal_size+1]
    for mfid in count:
        mask[mfid[0]] = 1
    print(mask)
    return mask

def get_random_mf_signal_mask(sim, signal_ratio, signal_size=None):
    mask = [0]*len(sim.mfs)
    mask_len = int(signal_ratio*len(sim.mfs))
    if signal_size is not None:
        mask_len = int(signal_size*len(sim.mfs))
...
def test(g, seed):
    ...
    if config.signal_type == 'top':
        signal_mask = get_top_mf_signal_mask(input_graph, config.signal_ratio, signal_size=config.signal_size)
    elif config.signal_type == 'bottom':
        signal_mask = get_top_mf_signal_mask(input_graph, config.signal_ratio, bottom=True, signal_size=config.signal_size)
    elif config.signal_type == 'random':
        signal_mask = get_random_mf_signal_mask(input_graph, config.signal_ratio, signal_size=config.signal_size)
    # print(signal_mask)
    else:
        assert False

    return test_across_noise(
        g,
        print_output=True,
...
save_fname = f'{script_n}/{script_n}_{model}_{n_grcs}_{n_mfs}_signal_ratio_{config.signal_ratio}_signal_type_{config.signal_type}'
if config.signal_size is not None:
    save_fname += f'_signal_size_{config.signal_size}'
save_fname += f'_{config.activation_level}_{config.pattern_len}_{config.n_random}'
print(save_fname)

...
ap.add_argument("--dendrite_length", type=int, help='', default=21)
...
fname_precalibrated = f'models/naive_random3_{config.n_grcs}_{config.n_mfs}_len_{config.dendrite_length}_seed_{config.seed}_precalibrated.gz'
fname = f'models/naive_random3_{config.n_grcs}_{config.n_mfs}_len_{config.dendrite_length}_f_{int(config.activation_level*1000)}_seed_{config.seed}'

if config.model == "small":
    input_graph = compress_pickle.load('/n/groups/htem/Segmentation/shared-nondev/cb2_segmentation/analysis_mf_grc/mf_grc_model/input_graph_210407_xlim_110.0_130.0_zlim_3.0_40.0_rep_3_wrapz_local_random.gz')
elif config.model == "big":
    input_graph = compress_pickle.load('/n/groups/htem/Segmentation/shared-nondev/cb2_segmentation/analysis_mf_grc/mf_grc_model/input_graph_210407_xlim_100.0_130.0_zlim_5.0_38.0_rep_3_wrapz_local_random.gz')

random.seed(config.seed)

# tools_mf_graph.shuffle(input_graph, "local_random")
# input_graph.randomize_graph_by_grc(
#     single_connection_per_pair=True,
#     constant_grc_degree=4,
#     constant_dendrite_length=config.dendrite_length*1000,
#     mf_dist_margin=4000,
#     )
g = ScaleUpModel(

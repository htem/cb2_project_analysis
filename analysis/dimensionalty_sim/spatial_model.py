...
class SpatialModel():
    ...
    def init_mfs(self):
        ...
        expanded_n_mfs = int(self.n_mfs*expansion_scale)
        print(f'expanded_size: {self.expanded_size}')
        print(f'Making {expanded_n_mfs} MFs')
        for i in range(expanded_n_mfs):
            mf = MossyFiber()
            self.mfs[i] = mf
    ...
    def get_possible_boutons(self, grc, dendrite_len):
        ...
        if len(ret) == 0:
            for i in range(dendrite_len-2*self.len_threshold, dendrite_len+2*self.len_threshold):
                ret.extend(grc.distance_cache[i])

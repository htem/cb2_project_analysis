...
ap.add_argument("--synapse_fail_rates", type=float, help='', default=None)
...
if config.synapse_fail_rates is None:
    step = .025
    mult=10000
    step_int = int(step*mult)
    config.synapse_fail_rates = [k/mult for k in range(step_int, int((1+step)*mult), step_int)]

print(config.synapse_fail_rates)

test_fn = functools.partial(run_tests_210617.test_across_synapse_fail_rates,
                            synapse_fail_rates=config.synapse_fail_rates,
                            grc_scale=config.grc_scale,
                            )

# save_args = (f"fail_{config.synapse_failure}_zmargin_{config.mfs_z_margin}")
save_args = (f"scale_{config.grc_scale}_")

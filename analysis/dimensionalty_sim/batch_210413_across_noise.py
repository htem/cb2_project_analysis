...
'''
python3 batch_210413_across_noise.py --model scaleup4 --n_random 100
python3 batch_210413_across_noise.py --model naive_random4 --n_random 100
'''
...
for n in range(config.n_random):
    ...
    compress_pickle.dump((
        ress,
        ), f"{script_n}/{script_n}_{model}_{config.n_grcs}_{config.n_mfs}_scaled_noise_{config.scaled_noise}_core_noise_{config.core_noise}_{config.activation_level}_{config.pattern_len}_{config.n_random}.gz")

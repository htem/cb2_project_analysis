...
fname_precalibrated = f'models/scaleup2_{config.n_grcs}_{config.n_mfs}_seed_{config.seed}_precalibrated.gz'
fname = f'models/scaleup2_{config.n_grcs}_{config.n_mfs}_f_{int(config.activation_level*1000)}_seed_{config.seed}'
...
input_graph = compress_pickle.load('/n/groups/htem/Segmentation/shared-nondev/cb2_segmentation/analysis_mf_grc/mf_grc_model/input_graph_210407_xlim_90_140_zlim_4.0_41.0_rep_4_wrapz.gz')
...
try:
    if config.overwrite:
        assert False
    g = compress_pickle.load(fname_precalibrated)
except:
    g = ScaleUpModel(

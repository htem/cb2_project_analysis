...
if variation_sizes is None:
    ...
    variation_sizes = [k/1000 for k in range(700, 1000, 10)]
...
noise_generator = functools.partial(add_noise_to_patterns, type=pattern_type, invert_noise_mask=False)

model_desc = None
def test(model, seed):
    global model_desc
    graph, model_desc = make_graph(model, seed=seed, n_grcs=n_grcs, n_mfs=n_mfs)
    sim = make_sim(graph)
